...
static mrb_code
bytecode_decoder(mrb_state* mrb, mrb_code code)
{
  if (stop_code) {
    return OP_STOP;
  }
...
RunMRuby()
{
  ...
  if (FR_OK == f_stat("code.mrb", &fno)) {
    is_bytecode = true;
  } else if (FR_OK != f_stat("code.rb", &fno)) {
    return 1;
  }

  code = (char *)malloc(fno.fsize);
  if (code == NULL) {
    printf("malloc failed\n");
    return 1;
  }

  fr = f_open(&fil, is_bytecode ? "code.mrb" : "code.rb", FA_READ);
  if (fr != FR_OK) {
    printf("f_open failed\n");
    free(code);
    return 1;
  }
  f_read(&fil, code, fno.fsize, &br);
  f_close(&fil);
  ...
  stop_code = false;
  mrb->bytecode_decoder = bytecode_decoder;

  if (is_bytecode)
    mrb_load_irep_buf(mrb, code, fno.fsize);
  else
    mrb_load_nstring(mrb, code, fno.fsize);

  mrb_close(mrb);
  free(code);

  return stop_code ? 1 : 0;
}
